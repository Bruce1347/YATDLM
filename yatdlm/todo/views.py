...
def display_list(request, list_id=-1, xhr=False, public=False):
    ...
    resolution_years = set(
        sorted(
            [task.resolution_date.year for task in all_tasks if task.resolution_date]
        )
    )
    deadlines_years = set(
        sorted([task.due_date.year for task in all_tasks if task.due_date])
    )
...
def list_tasks(request, list_id=None):
    try:
        ...
        todo = TodoList.objects.prefetch_related("task_set", "category_set").get(
            id=list_id
        )
        ...
        categories = [cat.as_dict() for cat in todo.category_set.all()]
        categories_str = [cat.name for cat in todo.category_set.all()]
        resp = {
            "tasks": [],
            "priorities": priorities,
        }
        subtasks = {}
        for sub in Task.objects.prefetch_related("parent_task").filter(
            parent_task_id__in=[task.id for task in tasks]
        ):
        ...
        for task in tasks:
            ...
            if task.id in subtasks:
                ...
                subtasks_done = [1 for sub in subtasks[task.id] if sub["is_done"]]
                task_dict["subtasks_progress"] = (
                    100.0 * sum(subtasks_done) / len(subtasks[task.id])
                )
            else:
...
def add_task(request, list_id=None):
    ...
    try:
        ...
        task = Task(
            ...
            due_date=datetime.strptime(body["due"], "%Y-%m-%d")
            if body["due"] != ""
            else None,
            owner=request.user,
            task_no=task_no,
...
def display_detail(request, list_id=-1, task_id=-1, add_followup=False, xhr=False):
    if task_id != -1 and list_id != -1:
        ...
        followups = FollowUp.objects.filter(task=task, todol_id=list_id).order_by(
            "creation_date"
        )

        return render(
            request,
            "todo/xhr/task_detail.html",
            {
                "task": task,
                "followups": followups,
                "xhr": xhr,
...
def add_list(request):
    from todo.schemas import TodoListSchema

    try:
        schema = TodoListSchema(**json.loads(request.body))
        import ipdb
        ...
        return JsonResponse(
            dict(errors=json.loads(e.json())), status=HTTPStatus.BAD_REQUEST
        )
